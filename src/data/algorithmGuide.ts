...
function findWords(board: string[][], words: string[]): string[] {
  ...
  },
  {
    ...
    label: '217. LeetCode 217. 存在重复元素',
    difficulty: '简单',
    description:
      '这题是在练集合判重。真正关键不是先排序再相邻比较，而是快速识别“是否出现过”，把问题直接收敛成哈希查重。',
    outcome:
      '你能掌握用哈希集合做一次遍历判重的思路，理解为什么这类题的核心是“是否见过”，并能写出最直接的线性解法。',
    sections: [
      {
        id: 'contains-duplicate-summary',
        title: '题目在问什么',
        summary: '给定一个整数数组，判断是否存在任意一个值至少出现两次。',
        bullets: [
          '只需要判断有无重复。',
          '不需要找出重复值具体是谁。',
          '也不要求统计重复次数。',
      ...
      {
        id: 'contains-duplicate-seen',
        title: '这题最核心的问题只有一个：当前数字是否已经出现过',
        summary:
          '一旦某个数字在之前已经见过，再次遇到它时答案就立刻确定为 `true`。所以最自然的状态就是维护一个“已经见过的数字集合”。',
        bullets: [
          '状态定义非常直接。',
          '问题天然适合在线处理。',
          '见过就返回，不必继续扫描。',
      ...
      },
      {
        ...
        title: '哈希集合比排序更贴题，因为它直接服务于“是否见过”',
        summary:
          '排序也能做，但它会重排数组并做额外工作。哈希集合只做当前题真正需要的事情：常数时间判断某个元素是否已出现过，因此思路更直接。',
        bullets: [
          '排序是间接解法。',
          '集合是直接解法。',
          '常数时间查找更契合题意。',
      ...
      {
        id: 'contains-duplicate-early-return',
        title: '因为只问真假，所以一旦发现重复就应立即返回',
        summary:
          '这题不需要把所有信息收集完整，只要发现一个重复值，答案就已经确定了。早返回能让逻辑更清楚，也体现出你对题目目标的把握。',
        bullets: [
          '真假题通常适合早返回。',
          '不需要继续做无意义遍历。',
          '逻辑会更简洁。',
        ...
        callout:
          '简单题真正要练的，往往不是技巧本身，而是你能不能把题目目标压缩成一个最直接的状态和动作。越简单的题，越能暴露思维是否清楚。',
      },
      {
        id: 'contains-duplicate-solution',
        title: '标准解法：哈希集合一遍扫描判重',
        summary:
          '创建一个集合，遍历数组中的每个数字。如果当前数字已经在集合中，直接返回 `true`；否则把它加入集合。遍历结束仍无重复，则返回 `false`。',
        bullets: [
          '时间复杂度是 `O(n)`。',
          '空间复杂度是 `O(n)`。',
          '写法最短也最稳定。',
  ...
  return false
      ...
      {
        id: 'contains-duplicate-mistakes',
        title: '易错点和延伸方向',
        summary:
          '这题最常见的问题，不是代码写错，而是思路绕远了，比如先排序再遍历，或者明明只要真假判断，却还去统计完整频次。',
        bullets: [
          '易错点 1：用过度复杂的解法处理简单判重题。',
          '易错点 2：忘了可以在发现重复时提前返回。',
          '易错点 3：把集合和数组查找混用，导致复杂度退化。',
          '延伸方向：存在重复元素 II、频次统计、哈希判重题。',
        ],
      },
    ],
  },
  ...
  },
  {
    ...
    label: '226. LeetCode 226. 翻转二叉树',
    difficulty: '简单',
    description:
      '这题是在练树的局部交换。真正关键不是把节点值倒过来，而是对每个节点交换左右子树，让整棵树的结构镜像翻转。',
    outcome:
      '你能掌握二叉树递归遍历中的局部处理方式，理解为什么交换左右子树后再递归或先递归再交换都成立，并能写出稳定解法。',
    sections: [
      {
        id: 'invert-binary-tree-summary',
        title: '题目在问什么',
        summary: '给定一棵二叉树，返回它的镜像翻转结果。',
        bullets: [
          '要翻转的是树结构。',
          '不是只交换节点值。',
          '每个节点的左右子树都要互换。',
      ...
      {
        id: 'invert-binary-tree-local',
        title: '这题最核心的动作，其实只有一个：交换当前节点的左右孩子',
        summary:
          '无论整棵树多大，翻转的局部规则始终不变：当前节点把左子树和右子树交换。整题之所以适合递归，就是因为全局任务可以自然拆成对子树做同样的事情。',
        bullets: [
          '局部规则极其简单。',
          '整棵树由同一规则递归组成。',
          '这很符合递归定义。',
      ...
      {
        id: 'invert-binary-tree-recursion',
        title: '递归思路就是：先翻左右子树，再把它们接反过来',
        summary:
          '把左子树递归翻好，把右子树递归翻好，然后把它们交换给当前节点；或者先交换再递归下去，本质上都成立，因为左右子树最终都会各自完成镜像翻转。',
        bullets: [
          '递归前后顺序都可以设计。',
          '关键是最终每个节点都完成交换。',
          '子问题与原问题结构完全一致。',
      ...
      {
        id: 'invert-binary-tree-bfs',
        title: '如果不用递归，层序遍历也能做，本质上仍是逐节点交换',
        summary:
          '把所有节点放进队列逐个处理，每取出一个节点，就交换它的左右孩子，再把孩子入队继续处理。它和递归的差别只在遍历组织形式，核心动作完全一样。',
        bullets: [
          'BFS 是迭代版实现。',
          '交换动作和递归版相同。',
          '适合不想用调用栈时使用。',
        ...
        callout:
          '简单树题最值得练的，是把“局部对、全局就对”的感觉建立起来。你能自然看出每个节点该做什么，递归就不再神秘。',
      },
      {
        id: 'invert-binary-tree-solution',
        title: '标准解法：递归交换左右子树',
        summary:
          '若当前节点为空，直接返回 `null`。否则先递归翻转左右子树，再把翻转后的右子树接到左边、左子树接到右边，最后返回当前节点。',
        bullets: [
          '时间复杂度是 `O(n)`。',
          '空间复杂度取决于树高。',
          '代码很短，递归结构清楚。',
  ...
  return root
      ...
      {
        id: 'invert-binary-tree-mistakes',
        title: '易错点和延伸方向',
        summary:
          '这题最常见的问题，不是不会递归，而是把“翻转结构”误写成“交换节点值”，或者在递归与交换顺序上把临时变量处理乱了。',
        bullets: [
          '易错点 1：错误地只交换值不交换子树引用。',
          '易错点 2：交换时没有临时保存，覆盖掉原引用。',
          '易错点 3：空节点返回条件漏掉。',
          '延伸方向：对称二叉树、树的镜像、递归与 BFS 对照题。',
        ],
      },
    ],
  },
  ...
  {
    id: 'power-of-two',
    label: '231. LeetCode 231. 2 的幂',
    difficulty: '简单',
    description:
      '这题是在练位运算判定。真正关键不是不断除以 2 去试，而是看出 2 的幂在二进制里只会有一个 `1`，其余位全是 `0`。',
    outcome:
      '你能掌握判断 2 的幂的二进制特征，理解为什么 `n & (n - 1)` 能消掉唯一的那个 `1`，并能写出简洁的位运算解法。',
    sections: [
      {
        id: 'power-of-two-summary',
        title: '题目在问什么',
        summary: '给定一个整数，判断它是否是 2 的幂。',
        bullets: [
          '只需要返回真假。',
          '2 的幂包括 `1, 2, 4, 8...`。',
          '负数和 0 都不合法。',
      ...
      {
        id: 'power-of-two-binary',
        title: '2 的幂在二进制中有一个极其稳定的特征：只会有一位是 `1`',
        summary:
          '例如 `1` 是 `1`，`2` 是 `10`，`4` 是 `100`，`8` 是 `1000`。它们共同点都是只有一个二进制位为 `1`。反过来，只要一个正整数二进制里不止一个 `1`，它就不可能是 2 的幂。',
        bullets: [
          '单个 `1` 是核心特征。',
          '其余位必须全为 0。',
          '这比循环试除更本质。',
      ...
      {
        id: 'power-of-two-n-and-n-minus-1',
        title: '表达式 `n & (n - 1)` 正好能测试“是否只有一个 `1`”',
        summary:
          '对于一个只有一个 `1` 的数，减 1 后这位 `1` 会变成 `0`，其右侧全变成 `1`。再与原数做按位与，结果就会变成 0。若原数有多个 `1`，按位与后通常还会留下其它 `1`。',
        bullets: [
          '`n & (n - 1)` 会删除最低位 `1`。',
          '只含一个 `1` 的数删除后变成 0。',
          '多个 `1` 的数删完最低位后仍不为 0。',
      ...
      {
        id: 'power-of-two-positive',
        title: '为什么还必须额外判断 `n > 0`',
        summary:
          '因为 `0` 的二进制里没有那个唯一的 `1`，而且像负数在补码表示下也会破坏这个判定语义。题目要求的是正整数中的 2 的幂，所以先排除非正数是必要前提。',
        bullets: [
          '`0` 不是 2 的幂。',
          '负数更不在定义范围内。',
          '位技巧必须建立在正整数语义上。',
        ...
        callout:
          '很多位运算题的陷阱不在公式本身，而在你有没有先把定义域想清楚。只要边界不先处理，后面的漂亮技巧也可能被无效输入破坏。',
      },
      {
        id: 'power-of-two-solution',
        title: '标准解法：正数且 `n & (n - 1) === 0`',
        summary:
          '先判断 `n` 是否大于 0。若不是，直接返回 `false`；若是，再检查 `n & (n - 1)` 是否等于 0。只有同时满足这两个条件，`n` 才是 2 的幂。',
        bullets: [
          '时间复杂度是 `O(1)`。',
          '空间复杂度是 `O(1)`。',
          '代码极短但很有代表性。',
  ...
  return n > 0 && (n & (n - 1)) === 0
      ...
      {
        id: 'power-of-two-mistakes',
        title: '易错点和延伸方向',
        summary:
          '这题最常见的问题，是只记住了 `n & (n - 1) === 0`，却忘了先判断正数；或者虽然能写出公式，却说不清它为什么能识别唯一 `1`。',
        bullets: [
          '易错点 1：漏掉 `n > 0` 条件。',
          '易错点 2：不会解释位运算原理。',
          '易错点 3：把题目做成循环试除但失去位题价值。',
          '延伸方向：3 的幂、4 的幂、位 1 个数、位运算判定题。',
        ],
      },
    ],
  },
  ...
  },
  {
    ...
    label: '237. LeetCode 237. 删除链表中的节点',
    difficulty: '中等',
    description:
      '这题是在练受限条件下的链表修改。真正关键不是按常规删除去找前驱，而是意识到题目根本不给前驱，所以只能把后继节点的值和指针关系覆盖到当前节点上。',
    outcome:
      '你能理解这题和普通链表删除的本质区别，掌握用后继节点覆盖当前节点的标准技巧，并能解释为什么它能等价完成删除。',
    sections: [
      {
        id: 'delete-node-in-a-linked-list-summary',
        title: '题目在问什么',
        summary:
          '给定一个链表中的某个非尾节点，要求在不知道头节点的情况下把它从链表中删除。',
        bullets: [
          '不会给链表头节点。',
          '给到的是待删节点本身。',
          '这个节点保证不是尾节点。',
      ...
      {
        id: 'delete-node-in-a-linked-list-difference',
        title: '这题和普通删除最大的不同，是你根本拿不到前驱节点',
        summary:
          '常规链表删除本质上是修改前驱的 `next` 指针，让它跳过当前节点。但这里题目只给了当前节点，不给头节点，也就没法从头找到前驱，因此常规思路走不通。',
        bullets: [
          '普通删除依赖前驱引用。',
          '本题条件直接切断了这条路。',
          '必须换个角度看待“删除”这件事。',
      ...
      {
        id: 'delete-node-in-a-linked-list-copy',
        title: '真正可行的做法，是把后继节点“拷贝过来”，再跳过后继',
        summary:
          '既然删不掉当前节点本身，那就把后继节点的值复制到当前节点里，再让当前节点的 `next` 指向后继的下一个节点。这样从链表外部看，原后继节点被移除了，而当前节点等价地变成了后继节点。',
        bullets: [
          '删除动作被转化成“覆盖加跳过”。',
          '链表外部观察到的结果与真删除等价。',
          '核心依赖于目标节点不是尾节点。',
      ...
      {
        id: 'delete-node-in-a-linked-list-why',
        title: '为什么这能等价于删除原节点',
        summary:
          '严格说，被物理移除的是后继节点，不是当前节点本体。但由于当前节点的值和后继完全一致，且链表连接关系也改成直接跳过后继，所以从外部能访问到的链表内容看，原目标节点已经“消失”了。',
        bullets: [
          '逻辑删除关注的是链表可观察结果。',
          '物理删除对象和语义删除对象可以不同。',
          '这是很多人第一次会觉得别扭的地方。',
        ...
        callout:
          '这题特别适合训练一个思维：当标准操作路径被限制住时，不一定非得硬走原路线，而是可以寻找一个对外效果等价的替代动作。',
      },
      {
        id: 'delete-node-in-a-linked-list-solution',
        title: '标准解法：用后继节点覆盖当前节点并跳过后继',
        summary:
          '取出当前节点的下一个节点，把它的值复制到当前节点，再让当前节点的 `next` 指向后继节点的下一个节点。这样就等价于把原给定节点从链表中删除了。',
        bullets: [
          '时间复杂度是 `O(1)`。',
          '空间复杂度是 `O(1)`。',
          '代码极短，但前提条件很关键。',
        ...
        code: `function deleteNode(node: ListNode | null): void {
  node!.val = node!.next!.val
  node!.next = node!.next!.next
}`,
      },
      {
        id: 'delete-node-in-a-linked-list-mistakes',
        title: '易错点和延伸方向',
        summary:
          '这题最常见的问题，是还在想着“怎么找到前驱”，说明没真正接受题目限制；或者忽略了“节点不是尾节点”这个前提，导致对尾节点误用这套覆盖法。',
        bullets: [
          '易错点 1：继续按普通删除去想前驱。',
          '易错点 2：没理解删除的是后继但效果等价。',
          '易错点 3：忽略非尾节点前提。',
          '延伸方向：链表节点覆盖技巧、受限操作问题、链表删除综合题。',
        ],
      },
    ],
  },
  ...
  },
  {
    ...
    label: '246. LeetCode 246. 中心对称数',
    difficulty: '简单',
    description:
      '这题是在练双指针对称映射。真正关键不是看一个数是不是回文，而是看它旋转 180 度之后，左右数字能不能两两映射成合法的对应关系。',
    outcome:
      '你能掌握中心对称数的字符映射规则，理解为什么要从两端同时检查映射关系，并能写出双指针解法。',
    sections: [
      {
        id: 'strobogrammatic-number-summary',
        title: '题目在问什么',
        summary:
          '给定一个数字字符串，判断它旋转 180 度后是否仍然表示同一个数。',
        bullets: [
          '不是普通回文题。',
          '考察的是旋转后的映射关系。',
          '输入按字符串处理更自然。',
      ...
      {
        id: 'strobogrammatic-number-mapping',
        title:
          '这题真正核心是那几组合法映射：`0-0`、`1-1`、`6-9`、`8-8`、`9-6`',
        summary:
          '旋转 180 度后，不是所有数字都还能变成合法数字。只有少数几种组合在旋转后能彼此对应，这些映射关系决定了整题的判断规则。',
        bullets: [
          '合法映射是有限且固定的。',
          '比如 `2`、`3`、`4`、`5`、`7` 都不合法。',
          '左右字符必须满足映射配对。',
      ...
      {
        id: 'strobogrammatic-number-two-pointers',
        title: '最自然的做法，是用双指针从两端同时检查',
        summary:
          '因为旋转后左边会对应到右边，右边也会对应回左边，所以一对字符必须一起看。双指针从首尾往中间收缩，正好符合这种成对验证方式。',
        bullets: [
          '左右字符是一组映射关系。',
          '双指针适合逐组检查。',
          '每次只验证一对字符是否合法。',
      ...
      {
        id: 'strobogrammatic-number-middle',
        title: '奇数长度时，中间那个字符也必须能自映射',
        summary:
          '如果字符串长度为奇数，最后会剩下中间一个字符。它不能依赖另一侧配对，所以必须是能旋转后仍指向自己的数字，也就是 `0`、`1` 或 `8`。',
        bullets: [
          '中间位没有配对对象。',
          '只能接受自映射字符。',
          '`6` 和 `9` 在中间都不合法。',
        ...
        callout:
          '很多对称题表面上看起来和回文像，但一旦把“相等”替换成“合法映射”，题目本质就变了。先认清约束类型，比直接套模板更重要。',
      },
      {
        id: 'strobogrammatic-number-solution',
        title: '标准解法：双指针验证首尾字符映射',
        summary:
          '准备一张映射表，记录每个字符旋转后应对应的字符。双指针从两端往中间移动，每次检查左字符映射后是否等于右字符；若有任何不匹配，直接返回 `false`，全部通过则返回 `true`。',
        bullets: [
          '时间复杂度是 `O(n)`。',
          '空间复杂度是 `O(1)`。',
          '代码非常短，但约束识别很关键。',
  ...
  while (left <= right) {
    const mapped = pairs.get(num[left])

    if (mapped === undefined || mapped !== num[right]) {
      return false
    }
  ...
  return true
      ...
      {
        id: 'strobogrammatic-number-mistakes',
        title: '易错点和延伸方向',
        summary:
          '这题最常见的问题，是把它误写成普通回文判断，忽略了 `6` 和 `9` 的交叉映射；或者在奇数长度时错误地允许中间位是 `6` 或 `9`。',
        bullets: [
          '易错点 1：把旋转对称误当成左右相等。',
          '易错点 2：中间位合法性判断错误。',
          '易错点 3：非法字符没有及时拦截。',
          '延伸方向：生成中心对称数、回文与对称映射对比题、双指针映射题。',
        ],
      },
    ],
  },
  ...
  {
    id: 'meeting-rooms',
    label: '252. LeetCode 252. 会议室',
    difficulty: '简单',
    description:
      '这题是在练区间冲突判断。真正关键不是把每个会议单独看，而是先按开始时间排序，再检查相邻会议是否发生重叠。',
    outcome:
      '你能掌握判断区间是否冲突的基本思路，理解为什么排序后只需比较相邻区间，并能写出标准解法。',
    sections: [
      {
        id: 'meeting-rooms-summary',
        title: '题目在问什么',
        summary: '给定一组会议时间区间，判断一个人能否参加所有会议。',
        bullets: [
          '一个人同一时间只能参加一个会议。',
          '只需要判断是否存在冲突。',
          '返回真假，不用找具体冲突对。',
      ...
      {
        id: 'meeting-rooms-sort',
        title: '判断区间是否冲突，第一步通常就是按开始时间排序',
        summary:
          '如果区间是乱序的，直接比较会很不直观。排序后，可能与当前会议冲突的，最值得优先看的就是前一个结束时间最近的会议，也就是相邻区间。',
        bullets: [
          '排序能把比较关系变局部。',
          '相邻区间最容易暴露冲突。',
          '不需要全对全比较。',
      ...
      {
        id: 'meeting-rooms-overlap',
        title: '排序后只要当前会议开始时间早于前一个会议结束时间，就冲突了',
        summary:
          '按开始时间排好后，若某个会议的开始时间小于前一个会议的结束时间，说明两者时间段存在交叠。只要出现一次这种情况，就不可能参加所有会议。',
        bullets: [
          '冲突判断条件非常直接。',
          '等于前一个结束时间通常不算冲突。',
          '出现一次冲突即可提前返回。',
      ...
      {
        id: 'meeting-rooms-why-adjacent',
        title: '为什么只比较相邻区间就够了',
        summary:
          '排序后，任何更后面的会议开始时间都不会早于当前相邻会议的开始时间。如果当前区间已经不和前一个重叠，那么它更不可能和更早但结束更早的区间形成新的更紧冲突。',
        bullets: [
          '排序把重叠风险集中到相邻关系上。',
          '前一个区间是最近的潜在冲突源。',
          '这就是比较数量能降到线性的原因。',
        ...
        callout:
          '区间题里“先排序再只看相邻”是非常值得内化的套路。很多看似全局比较的问题，一旦顺序固定，复杂度就能大幅下降。',
      },
      {
        id: 'meeting-rooms-solution',
        title: '标准解法：排序后线性扫描相邻会议',
        summary:
          '先按开始时间对区间排序。然后从第二个会议开始，逐个比较它的开始时间是否小于前一个会议的结束时间；若是，返回 `false`，否则继续扫描。若全部通过，则返回 `true`。',
        bullets: [
  ...
  return true
      ...
      {
        id: 'meeting-rooms-mistakes',
        title: '易错点和延伸方向',
        summary:
          '这题最常见的问题，是没排序就直接比较，或者把边界条件写成 `<=`，从而把前一个会议刚结束、下一个刚开始的情况也误判成冲突。',
        bullets: [
          '易错点 1：省略排序导致比较无效。',
          '易错点 2：冲突判断边界写错。',
          '易错点 3：明明是假题却做成复杂结构。',
          '延伸方向：会议室 II、区间合并、扫描线区间题。',
        ],
      },
    ],
  },
  ...
  {
    id: 'add-digits',
    label: '258. LeetCode 258. 各位相加',
    difficulty: '简单',
    description:
      '这题是在练数位规律。真正关键不是一轮轮拆位模拟，而是看出反复求数位和最终会收敛到数字根，可以直接用数学公式求解。',
    outcome:
      '你能掌握数字根的规律，理解为什么最终结果和 `mod 9` 有关，并能写出 `O(1)` 的解法。',
    sections: [
      {
        id: 'add-digits-summary',
        title: '题目在问什么',
        summary:
          '给定一个非负整数，反复把各位数字相加，直到结果只剩一位，返回这个最终数字。',
        bullets: [
          '过程会重复进行多轮。',
          '目标是最终的一位数。',
          '输入是非负整数。',
      ...
      {
        id: 'add-digits-simulate',
        title: '直接模拟当然能做，但题目更想考的是背后的数学规律',
        summary:
          '每轮把数字拆位、求和、再重复，逻辑上完全可行。但如果只停留在模拟层面，你会错过这题最有价值的地方：结果其实有稳定的闭式规律。',
        bullets: [
          '模拟法是可行基线。',
          '但复杂度和代码都不是最优。',
          '题目更看重规律识别。',
      ...
      {
        id: 'add-digits-digital-root',
        title: '反复求数位和，本质上是在求数字根',
        summary:
          '一个数对 9 取模的结果，与它各位数字之和对 9 取模的结果相同。反复做下去，最终收敛到的那个一位数，就是所谓的数字根，它和模 9 关系非常稳定。',
        bullets: [
          '数位和与原数在模 9 下等价。',
          '反复求和不会改变这个模 9 关系。',
          '最终一位数就是数字根。',
      ...
      {
        id: 'add-digits-formula',
        title: '公式要分两类：0 特判，其余用 `(num - 1) % 9 + 1`',
        summary:
          '若 `num` 为 0，答案就是 0；否则数字根在 `1` 到 `9` 之间循环，可以用 `(num - 1) % 9 + 1` 直接得到。这样既避开了 `9` 的边界问题，又统一了所有正整数情况。',
        bullets: [
          '0 是唯一需要单独处理的值。',
          '正整数结果不会落在 0。',
          '偏移写法能正确处理 9 的倍数。',
        ...
        callout:
          '很多简单数学题真正值钱的地方，不在于会不会写循环，而在于你能不能把重复过程识别成一个稳定不变量，再直接跳到结论。',
      },
      {
        id: 'add-digits-solution',
        title: '标准解法：数字根公式直接返回',
        summary:
          '先判断 `num` 是否为 0；若是，返回 0。否则使用 `(num - 1) % 9 + 1` 计算数字根并返回。这就是反复数位求和后的最终结果。',
        bullets: [
          '时间复杂度是 `O(1)`。',
          '空间复杂度是 `O(1)`。',
          '代码非常短。',
  ...
  return ((num - 1) % 9) + 1
      ...
      {
        id: 'add-digits-mistakes',
        title: '易错点和延伸方向',
        summary:
          '这题最常见的问题，是虽然知道和模 9 有关，却没有处理 `0` 和 9 的倍数边界，直接写成 `num % 9`，结果在这些特殊值上出错。',
        bullets: [
          '易错点 1：忽略 0 的特判。',
          '易错点 2：把 9 的倍数误算成 0。',
          '易错点 3：停留在模拟法没看到公式。',
          '延伸方向：快乐数、数位 DP、模运算规律题。',
        ],
      },
    ],
  },
  ...
  },
  {
    ...
    label: '260. LeetCode 260. 只出现一次的数字 III',
    difficulty: '中等',
    description:
      '这题是在练位运算分组。真正关键不是哈希统计全部频次，而是先用异或把两个只出现一次的数揉在一起，再借助它们某一位不同这一事实把数组分成两组。',
    outcome:
      '你能掌握用异或和最低位 1 做分组的思路，理解为什么两个目标数一定能被某一位区分开，并能写出 `O(1)` 额外空间解法。',
    sections: [
      {
        id: 'single-number-iii-summary',
        title: '题目在问什么',
        summary:
          '给定一个整数数组，其中恰好有两个元素只出现一次，其余元素都出现两次，返回这两个只出现一次的元素。',
        bullets: [
          '目标数恰好有两个。',
          '其余数都恰好出现两次。',
          '答案顺序不限。',
      ...
      {
        id: 'single-number-iii-xor',
        title: '第一步先整体异或，得到的其实是这两个目标数的异或值',
        summary:
          '因为成对出现的数字异或后都会抵消为 0，所以整数组异或的结果只剩下那两个只出现一次的数字的异或值。这个值虽然不是答案本身，但已经保留了它们的差异信息。',
        bullets: [
          '成对数字会完全抵消。',
          '最终异或值只包含两个目标数差异。',
          '这是后续分组的入口。',
      ...
      {
        id: 'single-number-iii-diff-bit',
        title: '两个不同数字的异或值中，至少有一位是 1，这一位就是分组依据',
        summary:
          '如果 `a ^ b` 的某一位为 1，说明 `a` 和 `b` 在这一位上不同。抓住任意这样一位，就能把数组里所有数字分成两组：这一位为 0 的一组、为 1 的一组，而两个目标数一定会被分到不同组。',
        bullets: [
          '1 位代表两个目标数在该位不同。',
          '这提供了天然的分组标准。',
          '只要能把两目标数拆开，问题就回到单个只出现一次。',
          '这是整题最关键的转折。',
        ],
      },
      {
        id: 'single-number-iii-group-xor',
        title: '分组后，每组里其余重复数字仍会抵消，最后只剩一个目标数',
        summary:
          '因为每个重复数字两次出现时，其对应位相同，所以它们一定会落入同一组，并在组内继续通过异或抵消。这样两组各自异或完后，就分别得到一个只出现一次的数字。',
        bullets: [
          '重复数不会被拆进不同组。',
          '组内异或仍然有效。',
          '两组最终各保留一个目标数。',
        ...
        callout:
          '位运算题最值得学的，往往不是某个公式本身，而是“如何利用一个区分位把全局问题切成两个更简单的问题”。这题就是非常标准的案例。',
      },
      {
        id: 'single-number-iii-solution',
        title: '标准解法：整体异或、提取区分位、分组异或',
        summary:
          '先计算所有数字的异或值 `xorAll`，再通过 `xorAll & -xorAll` 提取最低位的 1 作为区分位。之后按该位是否为 1 把所有数分组，分别异或两组元素，最终得到那两个只出现一次的数字。',
        bullets: [
          '时间复杂度是 `O(n)`。',
          '额外空间复杂度是 `O(1)`。',
          '是这题最经典的位运算解法。',
  ...
  return [first, second]
      ...
      {
        id: 'single-number-iii-mistakes',
        title: '易错点和延伸方向',
        summary:
          '这题最常见的问题，是虽然知道整体异或，但不会继续利用区分位分组；或者分组条件写错，导致两个目标数没被拆开。',
        bullets: [
          '易错点 1：停在 `xorAll` 不知道下一步怎么做。',
          '易错点 2：区分位提取或分组条件写错。',
          '易错点 3：误以为要排序或哈希统计。',
          '延伸方向：只出现一次的数字 I/II、位运算分组题、异或技巧题。',
        ],
      },
    ],
  },
...
ORDER BY t.request_at;`,
  ...
  {
    id: 'ugly-number',
    label: '263. LeetCode 263. 丑数',
    difficulty: '简单',
    description:
      '这题是在练质因子剥离。真正关键不是去判断一个数所有可能因子，而是利用定义只允许 2、3、5，持续把这些合法因子除干净，最后看是否只剩 1。',
    outcome:
      '你能掌握按允许质因子不断约简的思路，理解为什么最终能否化到 1 就决定了它是不是丑数，并能写出简洁解法。',
    sections: [
      {
        id: 'ugly-number-summary',
        title: '题目在问什么',
        summary:
          '给定一个整数，判断它是否是丑数。丑数定义为只包含质因子 `2`、`3`、`5` 的正整数。',
        bullets: [
          '定义只允许 2、3、5 作为质因子。',
          '必须是正整数。',
          '1 通常也视为丑数。',
      ...
      {
        id: 'ugly-number-strip',
        title: '最自然的判断方式，就是把 2、3、5 能除的都不断除掉',
        summary:
          '如果一个数真的只由 2、3、5 构成，那么不断用这三个数去除，最终一定会把它完全拆干净，剩下 1。反过来，如果最后还剩一个大于 1 的数，说明存在其它非法质因子。',
        bullets: [
          '合法质因子应当能被完全剥离。',
          '最后剩 1 说明结构纯净。',
          '剩余大于 1 就代表混入了其它质因子。',
      ...
      {
        id: 'ugly-number-order',
        title: '2、3、5 的剥离顺序其实无所谓，关键是都要除尽',
        summary:
          '因为乘法分解与顺序无关，所以先除 2 还是先除 5 都不影响最终结论。真正重要的是，对每个允许因子都要在可整除时持续除到不能再除为止。',
        bullets: [
          '顺序不影响结果。',
          '“除尽”比“除一次”更重要。',
          '这体现了完全分解的思想。',
      ...
      {
        id: 'ugly-number-positive',
        title: '为什么一开始就要排除非正数',
        summary:
          '题目定义的是正整数中的丑数，因此 0 和负数都不可能合法。先把这些情况排掉，后续剥离逻辑才有意义，也避免陷入无效循环。',
        bullets: [
          '定义域先行。',
          '0 和负数不属于候选集。',
          '这是最基础的边界条件。',
        ...
        callout:
          '很多看起来很“基础”的数论题，真正容易错的反而是定义边界。先把定义域想清楚，后面的规律才不会被无效输入干扰。',
      },
      {
        id: 'ugly-number-solution',
        title: '标准解法：循环剥离 2、3、5，最后判断是否为 1',
        summary:
          '若 `n <= 0`，直接返回 `false`。否则依次在 `n` 可被 2、3、5 整除时反复做除法。全部处理完后，如果 `n === 1`，说明它只含这三个质因子，是丑数；否则不是。',
        bullets: [
          '时间复杂度与可剥离次数有关。',
          '空间复杂度是 `O(1)`。',
          '代码很短，逻辑直接。',
  ...
  return n === 1
      ...
      {
        id: 'ugly-number-mistakes',
        title: '易错点和延伸方向',
        summary:
          '这题最常见的问题，是只把 2、3、5 各除一次就停止，或者漏掉 0、负数这类边界条件，导致定义都没守住。',
        bullets: [
          '易错点 1：没有把某个因子除尽。',
          '易错点 2：漏掉非正数边界。',
          '易错点 3：对 1 是否为丑数理解不清。',
          '延伸方向：丑数 II、质因子分解、受限质因子数论题。',
        ],
      },
    ],
  },
  {
    id: 'ugly-number-ii',
    label: '264. LeetCode 264. 丑数 II',
    difficulty: '中等',
    description:
      '这题是在练有序生成。真正关键不是从 1 开始逐个试数判断是不是丑数，而是直接按 2、3、5 的倍数生成下一个最小丑数序列。',
    outcome:
      '你能掌握用多指针动态生成丑数序列的思路，理解为什么三个指针分别代表下一个乘 2、乘 3、乘 5 的候选，并能写出标准解法。',
    sections: [
      {
        id: 'ugly-number-ii-summary',
        title: '题目在问什么',
        summary: '给定整数 `n`，返回第 `n` 个丑数。',
        bullets: [
          '丑数定义与前题相同。',
          '这次要求的是第 `n` 个值。',
          '不是判断某个数是否合法。',
      ...
      {
        id: 'ugly-number-ii-generate',
        title:
          '暴力一个个试是否为丑数可以做，但真正高效的方法是直接生成丑数序列',
        summary:
          '既然所有丑数都来自更小丑数乘以 2、3 或 5，那么我们没必要去检查所有普通整数。更自然的思路，是只在已知丑数基础上扩展出新的丑数。',
        bullets: [
          '丑数集合具有很强的递推结构。',
          '合法数由合法数乘 2/3/5 生成。',
          '无需把普通整数也纳入候选。',
      ...
      {
        id: 'ugly-number-ii-three-pointers',
        title: '三个指针分别代表“下一个乘 2、乘 3、乘 5 的候选来源”',
        summary:
          '维护一个丑数数组和三个指针 `i2`、`i3`、`i5`。它们分别指向当前还没被用来产生新丑数的最小位置，所对应的候选值是 `uglies[i2]*2`、`uglies[i3]*3`、`uglies[i5]*5`。',
        bullets: [
          '每个指针对应一个倍数来源。',
          '候选值始终来自已生成丑数。',
          '三者共同竞争下一个丑数。',
      ...
      {
        id: 'ugly-number-ii-deduplicate',
        title: '为什么命中最小值的多个指针都要一起前进',
        summary:
          '像 `6` 既可以由 `2*3` 生成，也可以由 `3*2` 生成。如果只前进一个指针，就会在后面重复产生同一个丑数。因此凡是候选值等于当前最小丑数的指针，都必须同步前进。',
        bullets: [
          '重复来源是多种乘法路径重合。',
          '同步前进是去重关键。',
          '否则序列会出现重复值。',
      ...
      {
        id: 'ugly-number-ii-ordered',
        title: '这套多指针方法能保证生成序列天然递增',
        summary:
          '因为每次都从三个候选中选最小值作为下一个丑数，而三个候选本身又都来自递增的已生成序列，所以最终构造出的整个丑数数组会按从小到大顺序产生。',
        bullets: [
          '最小候选选择保证当前最优。',
          '已生成序列递增保证候选有序。',
          '不需要额外排序。',
        ...
        callout:
          '很多数列生成题最值得训练的，不是会不会枚举，而是你能不能找到“下一个合法数一定来自哪些来源”。一旦来源结构固定，序列就能被有序地直接构造出来。',
      },
      {
        id: 'ugly-number-ii-solution',
        title: '标准解法：丑数数组加三指针动态生成',
        summary:
          '初始化丑数数组第一个元素为 1，并维护三个指针 `i2`、`i3`、`i5`。每轮取三个候选值的最小值作为下一个丑数，再把所有命中该最小值的指针同步后移。循环直到生成第 `n` 个丑数。',
        bullets: [
          '时间复杂度是 `O(n)`。',
          '空间复杂度是 `O(n)`。',
          '是这题最标准也最经典的解法。',
  ...
  for (let index = 1; index < n; index += 1) {
    const next2 = uglies[i2] * 2
    const next3 = uglies[i3] * 3
    const next5 = uglies[i5] * 5
  ...
  return uglies[n - 1]
      ...
      {
        id: 'ugly-number-ii-mistakes',
        title: '易错点和延伸方向',
        summary:
          '这题最常见的问题，是只移动一个命中最小值的指针，导致重复丑数；或者仍然沿用前题的逐数判断思路，复杂度明显不够好。',
        bullets: [
          '易错点 1：重复值去重处理不完整。',
          '易错点 2：三个候选来源理解不清。',
          '易错点 3：把生成题做回筛选题。',
          '延伸方向：超级丑数、多指针 DP、数列构造题。',
        ],
      },
    ],
  },
  ...
  },
  {
    ...
    label: '266. LeetCode 266. 回文排列',
    difficulty: '简单',
    description:
      '这题不是让你真的构造回文串，而是判断一个字符串能不能重排成回文。重点在于抓住回文结构的本质：字符出现次数的奇偶性。',
    outcome:
      '你能独立判断任意字符串是否存在回文重排，并知道为什么“最多只能有一个奇数次字符”就是完整条件。',
    sections: [
      {
        id: 'palindrome-permutation-summary',
        title: '题目在问什么',
        summary:
          '给定一个字符串 `s`，判断是否存在某种重排方式，使它变成一个回文串。',
        bullets: [
          '不是判断原串是不是回文。',
          '允许重新排列字符顺序。',
          '返回布尔值即可。',
      ...
      {
        id: 'palindrome-permutation-structure',
        title: '回文的本质是左右对称，所以大部分字符出现次数必须是偶数',
        summary:
          '一个回文串从两侧往中间看是成对出现的，因此除了中间位置之外，其余字符都必须成双成对。换句话说，出现次数为奇数的字符最多只能有一个。',
        bullets: [
          '偶数长度回文不允许奇数次字符。',
          '奇数长度回文允许恰好一个奇数次字符。',
          '这个条件已经同时覆盖所有情况。',
      ...
      {
        id: 'palindrome-permutation-count',
        title: '实现时只需要统计频次，再统计奇数项数量',
        summary:
          '可以用 `Map` 统计每个字符出现次数，然后遍历频次，计算有多少个字符的出现次数是奇数。如果奇数项数量小于等于 1，就说明可以重排成回文。',
        bullets: [
          '哈希表是最自然的频次容器。',
          '统计阶段和判断阶段可以分开写。',
          '代码清晰，复杂度也足够低。',
      ...
      {
        id: 'palindrome-permutation-toggle',
        title: '也可以只维护奇偶状态，不必真的记录完整次数',
        summary:
          '如果只关心奇偶性，那么每遇到一个字符就把它在集合里的状态翻转一次。最终集合里剩下的字符，就是出现奇数次的字符。',
        bullets: [
          '第一次出现加入集合。',
          '第二次出现移出集合。',
          '最终集合大小就是奇数频次字符数。',
        ...
        callout:
          '很多统计题不需要完整数据，只要抓住“题目最后真正关心的是什么信息”。本题只关心奇偶，不关心具体次数，所以状态可以进一步压缩。',
      },
      {
        id: 'palindrome-permutation-solution',
        title: '标准解法：集合翻转奇偶状态',
        summary:
          '用一个 `Set` 记录当前出现奇数次的字符。遍历字符串时，如果字符已在集合中就删除，否则加入。遍历结束后，只要集合大小不超过 1，就能重排成回文。',
        bullets: [
          '时间复杂度是 `O(n)`。',
          '空间复杂度最坏是 `O(k)`，`k` 为字符种类数。',
          '实现非常短，但思路很纯粹。',
          '适合训练“频次奇偶性”判断模式。',
        ],
  ...
  return oddChars.size <= 1
      ...
      {
        id: 'palindrome-permutation-mistakes',
        title: '易错点和延伸方向',
        summary:
          '这题最容易犯的错，是只记住“奇数次字符最多一个”这个结论，却说不清它为什么成立。面试里如果只会背结论，通常说明没有真正理解回文结构。',
        bullets: [
          '易错点 1：把“原串是否回文”和“能否重排成回文”混掉。',
          '易错点 2：忘了偶数长度时不能出现奇数项。',
          '易错点 3：统计了完整频次却没提到奇偶才是关键。',
          '延伸方向：回文构造、回文划分、位运算奇偶压缩。',
        ],
      },
    ],
  },
  ...
  {
    id: 'missing-number',
    label: '268. LeetCode 268. 丢失的数字',
    difficulty: '简单',
    description:
      '这题是数组与数学思维的经典入门题。重点不是会不会排序，而是意识到题目已经给了一个完整范围 `0..n`，所以可以直接利用总和公式或异或性质做常数空间求解。',
    outcome:
      '你能独立写出这题的求和法与异或法，并理解为什么这类“完整集合缺一项”的题目优先考虑数学特征。',
    sections: [
      {
        id: 'missing-number-summary',
        title: '题目在问什么',
        summary:
          '给定一个长度为 `n` 的数组 `nums`，里面包含 `0` 到 `n` 之间的 `n` 个不同数字，恰好缺失一个，返回这个缺失的数字。',
        bullets: [
          '范围是完整的 `0..n`。',
          '数组长度是 `n`，但范围里有 `n + 1` 个数。',
          '只缺一个，且其余数字都不重复。',
          '目标是找出缺的那一个值。',
        ],
      },
      {
        id: 'missing-number-sort',
        title: '排序或哈希都能做，但它们没有利用题目最重要的结构',
        summary:
          '先排序再看哪个位置不匹配，或者放进集合后逐个查找，当然都能解题。但这题最有价值的地方，是你要能看出完整区间这一条件意味着可以直接用整体信息反推缺失值。',
        bullets: [
          '排序解法时间复杂度通常是 `O(n log n)`。',
          '哈希集合解法空间复杂度是 `O(n)`。',
          '二者都没有把范围 `0..n` 用足。',
          '面试里更推荐数学法或异或法。',
        ],
      },
      {
        ...
        title: '最直接的高效思路：总和减实际和',
        summary:
          '如果 `0..n` 全都在，总和应为 `n * (n + 1) / 2`。把这个理论总和减去数组实际总和，得到的差值就是缺失数字。',
        bullets: [
          '整体求和可以一次完成。',
          '差值天然对应缺项。',
          '逻辑简单，代码短。',
      ...
      },
      {
        ...
        title: '异或法同样经典，因为成对数字会互相抵消',
        summary:
          '利用 `a ^ a = 0`、`a ^ 0 = a` 的性质，把 `0..n` 的所有数与数组中的所有数一起异或。数组里存在的数字会全部抵消，最后剩下的就是缺失数字。',
        bullets: [
          '异或不需要担心求和溢出问题。',
          '同一数字异或两次会消失。',
          '未成对的那个值最终会留下。',
        ...
        callout:
          '面对“完整集合缺一个元素”这类题，第一反应最好不是遍历找谁不在，而是想：有没有一个全局特征可以让完整集合和现有集合直接相减、相消或对比。',
      },
      {
        id: 'missing-number-solution',
        title: '标准解法：总和公式',
        summary:
          '先计算理论总和 `n * (n + 1) / 2`，再减去数组实际总和，结果就是缺失值。实现简单，复杂度也最优，非常适合作为这题的主解。',
        bullets: [
          '时间复杂度是 `O(n)`。',
          '空间复杂度是 `O(1)`。',
          '代码可读性很好。',
  ...
  return expectedSum - actualSum
      ...
      {
        id: 'missing-number-mistakes',
        title: '易错点和延伸方向',
        summary:
          '这题常见问题不是不会写，而是做得太重。明明题目有完整范围和只缺一项的结构，却还是上排序或哈希，说明对题目条件的利用还不够敏感。',
        bullets: [
          '易错点 1：忽略范围 `0..n` 的数学结构。',
          '易错点 2：把下标和值混为一谈。',
          '易错点 3：只会求和法，不理解异或法为何可行。',
          '延伸方向：找重复数、找缺失区间、异或消元题。',
        ],
      },
    ],
  },
  ...
  {
    id: 'h-index',
    label: '274. LeetCode 274. H 指数',
    difficulty: '中等',
    description:
      '这题的关键不是公式，而是对 H 指数定义的准确翻译：至少有 `h` 篇论文的引用次数不少于 `h`。重点在于把这个条件和排序后的位置关系对应起来。',
    outcome:
      '你能通过排序法快速求出 H 指数，并理解为什么“当前位置后面还有多少篇论文”就对应了候选 `h` 值。',
    sections: [
      {
        id: 'h-index-summary',
        title: '题目在问什么',
        summary:
          '给定一个整数数组 `citations`，其中 `citations[i]` 表示某篇论文的引用次数，返回研究者的 H 指数。',
        bullets: [
          'H 指数是一个平衡数量和质量的指标。',
          '定义是：至少有 `h` 篇论文引用数不少于 `h`。',
          '其余论文引用数不超过 `h` 不影响答案。',
          '目标是找满足条件的最大 `h`。',
        ],
      },
      {
        id: 'h-index-sort',
        title: '排序后，问题会变成“当前位置是否支撑一个候选 h”',
        summary:
          '如果把引用次数按升序排序，那么扫描到位置 `i` 时，右侧包含当前元素在内一共有 `n - i` 篇论文，它们的引用次数都至少是 `citations[i]`。因此可以检查 `citations[i] >= n - i` 是否成立。',
        bullets: [
          '排序把数量关系固定下来了。',
          '`n - i` 是当前还能保证的论文篇数。',
          '若引用次数至少达到这个数量，就形成合法候选。',
          '我们要找满足条件的最大值。',
        ],
      },
      {
        id: 'h-index-why',
        title: '为什么返回 `n - i` 而不是 `citations[i]`',
        summary:
          'H 指数不是单纯看某篇论文被引用多少次，而是看“有多少篇论文都至少达到这个次数”。排序后，真正能被整体保证的篇数是右边这 `n - i` 篇，所以候选值是 `n - i`。',
        bullets: [
          'H 指数强调的是群体门槛。',
          '单篇极高引用不能直接决定 H。',
          '排序位置决定了可支撑的篇数。',
          '这也是很多人第一次做这题时最容易误解的地方。',
        ],
      },
      {
        id: 'h-index-linear-scan',
        title: '排序后从左到右找到第一个满足条件的位置即可',
        summary:
          '一旦在某个位置 `i` 满足 `citations[i] >= n - i`，说明右边这 `n - i` 篇论文都至少被引用了 `n - i` 次，这已经构成一个合法 H 指数。由于 `i` 越往右，`n - i` 越小，所以第一个满足条件的位置就对应最终答案。',
        bullets: [
          '升序扫描逻辑非常清晰。',
          '也可以降序写成另一种等价形式。',
          '代码量小，面试表达也直观。',
        ...
        callout:
          '这题很适合训练“把定义翻译成排序后的下标关系”。很多数组题看似数学定义复杂，真正落地时，只要把元素按某种顺序排好，条件就会变成非常机械的下标判断。',
      },
      {
        id: 'h-index-solution',
        title: '标准解法：排序后线性扫描',
        summary:
          '先把数组升序排序。然后从左到右遍历，检查当前引用数是否至少为 `n - i`。一旦满足，就返回 `n - i`。如果始终不满足，说明 H 指数为 0。',
        bullets: [
          '时间复杂度是 `O(n log n)`，主要来自排序。',
          '空间复杂度取决于排序实现，通常记作 `O(1)` 或 `O(log n)` 额外栈空间。',
          '实现简单且容易解释。',
          '非常适合作为 H 指数的入门写法。',
        ],
  ...
  return 0
      ...
      },
      {
        ...
        title: '易错点和延伸方向',
        summary:
          '这题最容易写错的地方，是把候选值误当成当前引用次数，而不是“至少能支撑的论文篇数”。一旦这点没想清楚，排序后判断式就会很乱。',
        bullets: [
          '易错点 1：没理解 H 指数定义中的“至少 h 篇”。',
          '易错点 2：排序后候选值取错。',
          '易错点 3：升序和降序写法混淆。',
          '延伸方向：H 指数 II、计数排序优化、定义转下标题。',
        ],
      },
    ],
  },
...
function solution(isBadVersion: (version: number) => boolean) {
  ...
  {
    id: 'wiggle-sort',
    label: '280. LeetCode 280. 摆动排序',
    difficulty: '中等',
    description:
      '这题看起来像排序题，但真正高效的做法并不是排完再重组，而是在线性扫描中就地修正相邻关系。重点是理解每个位置只需要满足局部波峰波谷条件。',
    outcome:
      '你能写出这题的单次遍历原地解法，理解为什么只维护相邻位置的局部摆动关系，就能得到全局合法结果。',
    sections: [
      {
        id: 'wiggle-sort-summary',
        title: '题目在问什么',
        summary:
          '给定一个数组 `nums`，原地重排，使其满足 `nums[0] <= nums[1] >= nums[2] <= nums[3] ...` 的摆动关系。',
        bullets: [
          '要求原地修改数组。',
          '目标是局部波峰波谷交替。',
          '不要求唯一答案。',
      ...
      {
        id: 'wiggle-sort-sort-way',
        title: '先排序再交换也能做，但不是最直接的最优思路',
        summary:
          '如果先排序，再把相邻元素成对调整，确实能得到一个合法摆动序列。但这需要 `O(n log n)` 排序时间，而题目其实可以在线性时间内完成。',
        bullets: [
          '排序法更容易想到。',
          '但它做了很多并不必要的全局工作。',
          '摆动要求只约束相邻关系。',
      ...
      {
        id: 'wiggle-sort-local-rule',
        title: '关键观察：只要当前位置和前一个位置不满足应有关系，就交换它们',
        summary:
          '从左到右扫描时，奇数位应当成为局部峰值，所以需要满足 `nums[i] >= nums[i - 1]`；偶数位应当成为局部谷值，所以需要满足 `nums[i] <= nums[i - 1]`。一旦不满足，就交换这两个相邻元素。',
        bullets: [
          '每一步只关心一对相邻元素。',
          '奇偶位置对应不同不等号方向。',
          '不满足就交换，满足就继续。',
      ...
      {
        id: 'wiggle-sort-why-work',
        title: '为什么局部交换不会破坏前面已经满足的关系',
        summary:
          '因为当前只和前一个元素交换，而前一个位置在进入当前轮次之前已经满足了它和更前面元素的摆动关系。结合当前奇偶位要求，可以证明这次局部修正不会把前缀整体结构破坏掉。',
        bullets: [
          '前缀在每一轮结束后保持合法。',
          '当前交换只影响最近两个位置。',
          '这是贪心正确性的核心。',
        ...
        callout:
          '很多数组重排题并不需要先得到某个全局排序结果，再做大动作调整。只要题目的合法性可以拆成一系列局部条件，贪心地边扫边修往往更高效。',
      },
      {
        id: 'wiggle-sort-solution',
        title: '标准解法：单次遍历，按奇偶位局部交换',
        summary:
          '从下标 `1` 开始遍历数组。若当前是奇数位且 `nums[i] < nums[i - 1]`，就交换；若当前是偶数位且 `nums[i] > nums[i - 1]`，也交换。扫描结束后，整个数组就满足摆动关系。',
        bullets: [
          '时间复杂度是 `O(n)`。',
          '空间复杂度是 `O(1)`。',
          '实现短小，但思路非常典型。',
  ...
  for (let index = 1; index < nums.length; index += 1) {
    const shouldBePeak = index % 2 === 1

    if (
      (shouldBePeak && nums[index] < nums[index - 1]) ||
      (!shouldBePeak && nums[index] > nums[index - 1])
    ) {
      ...
      {
        id: 'wiggle-sort-mistakes',
        title: '易错点和延伸方向',
        summary:
          '这题最容易出错的地方，是把不等号方向记反，或者误以为必须先全局排序。实际上题目只要求局部波动关系，没要求相对大小全局有序。',
        bullets: [
          '易错点 1：奇数位和偶数位的不等号写反。',
          '易错点 2：过度依赖排序，没看到线性贪心解。',
          '易错点 3：交换后担心破坏前缀，但说不清原因。',
          '延伸方向：Wiggle Sort II、局部贪心重排、数组模式构造。',
        ],
      },
    ],
  },
  ...
  {
    id: 'move-zeroes',
    label: '283. LeetCode 283. 移动零',
    difficulty: '简单',
    description:
      '这题是双指针数组原地操作的基础训练题。重点不是把零挪到后面这件事本身，而是建立“保留有效元素顺序，同时把无效元素压缩到末尾”的通用思维。',
    outcome:
      '你能写出稳定的原地双指针解法，理解为什么先压缩非零元素、再补零，是处理这类数组整理题的标准套路。',
    sections: [
      {
        id: 'move-zeroes-summary',
        title: '题目在问什么',
        summary:
          '给定一个数组 `nums`，要求原地把所有 `0` 移动到数组末尾，同时保持非零元素的相对顺序不变。',
        bullets: [
          '必须原地修改数组。',
          '非零元素相对顺序不能被打乱。',
          '所有 `0` 最终集中到后面。',
      ...
      {
        id: 'move-zeroes-bruteforce',
        title: '删除零再补零能做，但会产生不必要的数据移动',
        summary:
          '如果每遇到一个 `0` 就把它删掉，再在尾部补一个 `0`，虽然思路直观，但数组删除会触发大量元素左移，整体效率不高，也不够优雅。',
        bullets: [
          '频繁删除会导致多次搬移。',
          '时间复杂度容易退化。',
          '实现也比较繁琐。',
      ...
      {
        id: 'move-zeroes-two-pointers',
        title: '核心思路：一个指针读，一个指针写非零元素',
        summary:
          '用 `read` 指针从左到右扫描数组，用 `write` 指针表示下一个应该放置非零元素的位置。每遇到一个非零值，就把它写到 `write` 位置，然后 `write` 前进。',
        bullets: [
          '读指针负责遍历所有元素。',
          '写指针负责压缩后的有效区间。',
          '非零元素会被按原顺序写回前部。',
      ...
      {
        id: 'move-zeroes-fill-zero',
        title: '压缩完成后，剩余位置统一补零即可',
        summary:
          '当所有非零元素都已经按顺序写到数组前半部分后，`write` 之后的位置就是空出来的区域。只要把这些位置全部填成 `0`，题目要求就全部满足。',
        bullets: [
          '前半段是稳定压缩后的非零元素。',
          '后半段统一补零即可。',
          '整个过程只需两次线性扫描。',
        ...
        callout:
          '数组原地题经常不是“怎么把某类元素移到后面”，而是“怎么把有效元素稳定压缩到前面”。只要前半段稳定了，后半段通常都只是收尾动作。',
      },
      {
        id: 'move-zeroes-solution',
        title: '标准解法：双指针压缩非零元素',
        summary:
          '先遍历数组，把所有非零元素依次写到前面。写完后，再从 `write` 位置开始把剩余槽位补成 `0`。这样既保证了原地修改，又保留了非零元素顺序。',
        bullets: [
          '时间复杂度是 `O(n)`。',
          '空间复杂度是 `O(1)`。',
          '是这题最标准、最稳的写法。',
  ...
  while (write < nums.length) {
    ...
    write += 1
      ...
      {
        id: 'move-zeroes-mistakes',
        title: '易错点和延伸方向',
        summary:
          '这题最常见的错误，是虽然用了双指针，但没想清楚稳定性要求，结果在交换过程中把非零元素原有顺序打乱了。',
        bullets: [
          '易错点 1：忽略“保持相对顺序”这一要求。',
          '易错点 2：写指针更新时机错误。',
          '易错点 3：只压缩了非零元素，忘了补零。',
          '延伸方向：删除元素、移除重复项、稳定原地压缩。',
        ],
      },
    ],
  },
...
class PeekingIterator {
  ...
  {
    id: 'nim-game',
    label: '292. LeetCode 292. Nim 游戏',
    difficulty: '简单',
    description:
      '这题是博弈论入门经典题。重点不是背结论，而是观察小规模情况并归纳出必败态规律：每逢 4 的倍数，先手都会被逼入劣势。',
    outcome:
      '你能通过找规律理解这题为什么只需判断 `n % 4 !== 0`，并知道这种做法来自对先后手必胜必败态的归纳。',
    sections: [
      {
        id: 'nim-game-summary',
        title: '题目在问什么',
        summary:
          '桌上有 `n` 块石头，两名玩家轮流取走 1 到 3 块，谁取到最后一块谁赢。假设双方都绝顶聪明，判断先手是否必胜。',
        bullets: [
          '每次只能取 1、2、3 块。',
          '取走最后一块者获胜。',
          '双方都采用最优策略。',
      ...
      {
        id: 'nim-game-small-cases',
        title: '先从小规模手算，规律会很快浮现',
        summary:
          '当 `n = 1, 2, 3` 时，先手都能直接拿完获胜；当 `n = 4` 时，无论先手拿几块，都会给后手留下 `1、2、3` 中的一种，后手总能一把拿完。所以 `4` 是必败态。',
        bullets: [
          '`1、2、3` 都是先手必胜。',
          '`4` 是第一个必败态。',
          '观察到这里后应继续往后推。',
      ...
      {
        id: 'nim-game-induction',
        title: '一旦确认 4 是必败态，后面每隔 4 个就会重复一次',
        summary:
          '若当前是 `5、6、7`，先手都可以分别拿 `1、2、3` 块，把局面变成 `4` 留给后手，因此这三种都是必胜态。接着 `8` 又会成为必败态，因为无论先手怎么拿，都会把 `5、6、7` 这种必胜态交给对手。',
        bullets: [
          '必胜态是能一步走到必败态的状态。',
          '必败态是所有下一步都走向必胜态的状态。',
          '这样会形成以 4 为周期的模式。',
      ...
      {
        id: 'nim-game-core',
        title: '最终结论就是：先手只在 4 的倍数时输',
        summary:
          '如果 `n` 不是 4 的倍数，先手总能拿走适当数量，让剩余石头数变成 4 的倍数；之后无论后手拿多少，先手都能继续把局面拉回 4 的倍数，直到胜利。',
        bullets: [
          '控制局面落在 4 的倍数上是策略核心。',
          '这是典型的保持不变量思维。',
          '结论可以直接写成数学判断。',
        ...
        callout:
          '很多简单博弈题并不需要复杂 DP，先算几个最小状态，找出“必胜态/必败态”如何递推，往往就能直接抽象出一个周期规律或不变量。',
      },
      {
        id: 'nim-game-solution',
        title: '标准解法：判断是否为 4 的倍数',
        summary:
          '根据规律，若 `n` 是 `4` 的倍数，则先手必败；否则先手必胜。因此代码只需要返回 `n % 4 !== 0`。',
        bullets: [
          '时间复杂度是 `O(1)`。',
          '空间复杂度是 `O(1)`。',
          '代码极短，但前提是规律已经想清楚。',
  ...
  return n % 4 !== 0
      ...
      {
        id: 'nim-game-mistakes',
        title: '易错点和延伸方向',
        summary:
          '这题最常见的问题，是看到博弈就直接上递归或 DP，却没先尝试列前几个状态。对于这种规则固定、动作很少的小游戏，找规律往往比套模板更高效。',
        bullets: [
          '易错点 1：没先手推小规模状态。',
          '易错点 2：只背公式，不知道为什么是 4。',
          '易错点 3：把必胜态和必败态的递推关系说反。',
          '延伸方向：博弈论入门、状态归纳、不变量策略。',
        ],
      },
    ],
  },
  {
    id: 'flip-game',
    label: '293. LeetCode 293. 翻转游戏',
    difficulty: '简单',
    description:
      '这题本质是一个状态生成题。重点不是博弈分析，而是找到所有一步可达状态：只要把任意一对连续的 `++` 翻成 `--` 即可得到一个新局面。',
    outcome:
      '你能快速识别并枚举所有合法下一步状态，理解这类题的核心是模式扫描和状态构造。',
    sections: [
      {
        id: 'flip-game-summary',
        title: '题目在问什么',
        summary:
          '给定一个只包含 `+` 和 `-` 的字符串，允许把任意一对连续的 `++` 翻转成 `--`。返回所有可能的一步操作结果。',
        bullets: [
          '只做一步操作。',
          '只能翻连续的 `++`。',
          '每个可翻位置都产生一个结果。',
      ...
      {
        id: 'flip-game-scan',
        title: '做法就是从左到右扫描每一对相邻字符',
        summary:
          '只要当前位置和下一个位置都是 `+`，说明这里可以进行一次合法翻转。把这两个字符替换成 `--`，其余部分保持不变，就得到一个候选答案。',
        bullets: [
          '本质是相邻模式识别。',
          '每个合法位置对应一个结果。',
          '没有更复杂的状态依赖。',
      ...
      {
        id: 'flip-game-build',
        title: '字符串构造可以用切片拼接完成',
        summary:
          '当发现 `s[index]` 和 `s[index + 1]` 都是 `+` 时，答案字符串可以写成 `s.slice(0, index) + "--" + s.slice(index + 2)`。这种方式最直接，也容易读。',
        bullets: [
          '前缀保持不变。',
          '中间两位替换为 `--`。',
          '后缀接回原字符串剩余部分。',
      ...
      {
        id: 'flip-game-no-extra',
        title: '这题不需要回溯，因为只要求一步结果',
        summary:
          '很多人看到“所有可能状态”就条件反射想到回溯，但这里只要求一步操作后的全部结果，不需要继续向下搜索，所以单次线性扫描就足够了。',
        bullets: [
          '没有多层决策树。',
          '每个位置独立产出一个状态。',
          '不需要递归。',
        ...
        callout:
          '题目问“所有可能”时，不代表一定要回溯。你先判断它要求的是“一步可达状态集合”还是“多步搜索空间”，再决定用简单枚举还是递归搜索。',
      },
      {
        id: 'flip-game-solution',
        title: '标准解法：线性扫描并生成所有可翻状态',
        summary:
          '遍历字符串的每个相邻位置。如果出现 `++`，就用切片拼接构造对应翻转后的字符串并加入结果数组。遍历结束后返回结果。',
        bullets: [
          '时间复杂度约为 `O(n²)`，因为每次构造字符串需要复制。',
          '空间复杂度取决于结果数量。',
          '思路简单直接。',
          '是这题最标准的实现。',
        ],
        code: `function generatePossibleNextMoves(currentState: string): string[] {
  const result: string[] = []
  ...
  return result
      ...
      {
        id: 'flip-game-mistakes',
        title: '易错点和延伸方向',
        summary:
          '这题最常见的错误，是把题意想复杂了，上来就写递归。其实这里只要枚举一步操作结果，不涉及谁输谁赢，也不涉及多层搜索。',
        bullets: [
          '易错点 1：误用回溯。',
          '易错点 2：漏掉重叠位置的扫描。',
          '易错点 3：字符串切片边界写错。',
          '延伸方向：Flip Game II、状态生成、字符串枚举。',
        ],
      },
    ],
  },
